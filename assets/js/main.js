document.addEventListener("DOMContentLoaded", () => {
  ...
  // 默认语言：优先读 localStorage，否则检测浏览器
  const savedLang = localStorage.getItem("preferredLang");
  const browserLang = savedLang || (navigator.language.startsWith("zh") ? "zh" : "en");
  setLanguage(browserLang);

  // 语言切换按钮（同时写入 localStorage）
  document.getElementById("lang-en")?.addEventListener("click", () => { setLanguage("en"); localStorage.setItem("preferredLang", "en"); });
  document.getElementById("lang-zh")?.addEventListener("click", () => { setLanguage("zh"); localStorage.setItem("preferredLang", "zh"); });
